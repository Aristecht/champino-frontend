import type { Metadata } from "next";
import { HomePageClient } from "./HomePageClient";

export const metadata: Metadata = {
  title: "ЧАМПИНО ZOO — зоомагазин в Атырау",
  description:
    "Зоомагазин ЧАМПИНО ZOO в Атырау: корма, лакомства, наполнители и аксессуары для кошек, собак и других питомцев. Программа лояльности — 5 покупок даёт скидку 2%, 50 покупок — до 10%. Онлайн-заказ и быстрая доставка.",
  keywords: [
    "зоомагазин Атырау",
    "зоомагазин",
    "ЧАМПИНО ZOO",
    "товары для питомцев",
    "программа лояльности зоомагазин",
    "скидки на зоотовары",
    "карта лояльности питомцы",
    "корм для кошек Атырау",
    "корм для собак Атырау",
    "доставка зоотоваров",
  ],
  alternates: { canonical: "/" },
  openGraph: {
    title: "ЧАМПИНО — зоомагазин в Атырау",
    description:
      "Товары для питомцев с доставкой по Атырау. Программа лояльности: накапливайте покупки и получайте скидку до 10% на весь ассортимент.",
    url: "/",
    type: "website",
  },
};

export default function Page() {
  return <HomePageClient />;
}
